...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = torch.stack([self.efficient_net.extract_features(each) for each in xs], dim=1)  # 16 x [406, 1280, 5, 4] ->  [406, 16, 1280, 5, 4]
        #print("feature shape after efficient", x.shape)  # ([406, 1280, 5, 4])
        # ([406, 16, 1280, 5, 4]) # B, T, C, H, W
        layer_output_list, last_state_list = self.convlstm(self.efficient_net.extract_features(x.view(-1, 3, 170, 128)).view(-1, 16, 1280, 5, 4).float())
        # [406, 1, 128, 5, 4] this is the last hidden layer's output #.squeeze()
        # layer_output_list 3 1 torch.Size([406, 16, 128, 5, 4]) last_state_list 3 2 1 torch.Size([406, 128, 5, 4])
        # torch.Size([25, 16, 64, 5, 4])
        # torch.Size([25, 16, 64, 5, 4])
        #print("after unbind shape", x.shape)  # torch.Size([25, 64, 5, 4])
        x = torch.flatten(torch.unbind(layer_output_list[-1].half(), dim=1)[-1], 1)  # flatten all dimensions except batch # 在实际run_agent时候，这里没有把输出打平，而是
        # print("x shape after flatten", x.shape)  # flatten torch.Size([25, 20480])
        # ([time step size, 2560])
        x = self.leaky_relu(self.fc3(self.leaky_relu(self.fc2(self.leaky_relu(self.fc1(x))))))
